    ...
    <div className="bg-background">
      ...
      <section className="py-24 md:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="max-w-3xl mb-16">
            <span className="text-xs font-bold uppercase tracking-widest text-[#22c55e]">What&apos;s included</span>
            <h2 className="mt-4 text-4xl md:text-5xl font-bold text-slate-900 font-display text-balance">A full learning environment, built for the field.</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {FEATURES.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.title} className="p-6 rounded-3xl bg-slate-50 ring-1 ring-slate-200 hover:bg-white hover:ring-[#166534]/25 hover:shadow-lg transition-all">
                  <div className="grid size-11 place-items-center rounded-xl bg-white ring-1 ring-slate-200 text-[#166534]">
                    <Icon size={18} />
                  </div>
      ...
      <section className="py-24 md:py-32 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="max-w-3xl mb-14">
            <span className="text-xs font-bold uppercase tracking-widest text-[#22c55e]">Featured courses</span>
            <h2 className="mt-4 text-4xl md:text-5xl font-bold text-slate-900 font-display text-balance">Start where you are.</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {COURSES.map((c) => (
              <article key={c.title} className="group p-8 rounded-3xl bg-white ring-1 ring-slate-200 hover:ring-[#166534]/25 hover:shadow-lg transition-all">
                <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest">
                  <span className="text-[#22c55e]">{c.level}</span>
                  <span className="text-slate-300">&bull;</span>
                  <span className="text-slate-500">{c.duration}</span>
                </div>
                <h3 className="mt-3 text-xl font-bold text-slate-900 font-display leading-snug">{c.title}</h3>
                <button className="mt-6 text-[#166534] font-semibold hover:underline">View syllabus →</button>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 rounded-[36px] overflow-hidden">
          <img src={farmers} alt="Field training with farmers" loading="lazy" className="w-full aspect-[21/9] object-cover rounded-[36px]" />
        </div>
      </section>
