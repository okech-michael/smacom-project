...
⚠️ **Important**: SQLite is not recommended for production on Vercel. For production, use Supabase Postgres or another managed PostgreSQL service.

- Use **Supabase Postgres** for production database hosting
- Optionally use **AWS RDS**, **Neon**, or **PlanetScale** as alternatives
...
**Frontend Variables:**
- `VITE_API_URL`: `/api` when frontend and backend share the same production domain
- `VITE_APP_ID`: Application identifier (default: `smacom`)

**Backend Variables:**
- `DATABASE_URL`: Your production Supabase Postgres connection string
- `JWT_SECRET`: A strong, unique secret key (generate using `openssl rand -base64 32`)
- `CLIENT_URL`: `https://www.smacom.co.ke`
...
### From SQLite to PostgreSQL with Supabase

1. Create a Supabase project and Postgres database
2. Copy the Supabase connection string
3. Set `DATABASE_URL` in Vercel to the Supabase connection string
4. Run migrations locally or in a deployment step:
   ```bash
   cd backend
   npx prisma db push
   cd ..
